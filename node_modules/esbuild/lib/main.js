...
function createChannel(streamIn) {
  ...
  let handleIncomingPacket = (bytes) => {
    if (isFirstPacket) {
      ...
      if (binaryVersion !== "0.8.32") {
        throw new Error(`Cannot start service: Host version "${"0.8.32"}" does not match binary version ${JSON.stringify(binaryVersion)}`);
      }
      return;
    }
...
var startService = referenceCountedService(() => process.cwd(), (options) => {
  ...
  let child = child_process.spawn(command, args.concat(`--service=${"0.8.32"}`), {
    cwd: process.cwd(),
    windowsHide: true,
  ...
  return Promise.resolve({
    build: (options2) => refPromise(new Promise((resolve, reject) => service.buildOrServe("build", null, options2, isTTY(), (err, res) => err ? reject(err) : resolve(res)))),
    serve: (serveOptions, buildOptions) => {
...
var runServiceSync = (callback) => {
  ...
  callback(service);
  let stdout = child_process.execFileSync(command, args.concat(`--service=${"0.8.32"}`), {
    cwd: process.cwd(),
    windowsHide: true,
    input: stdin,
