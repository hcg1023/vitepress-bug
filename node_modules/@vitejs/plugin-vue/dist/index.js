...
async function transformMain(code, filename, options, pluginContext) {
  ...
  if (hasTemplateImport) {
    ;
    ({code: templateCode, map: templateMap} = genTemplateCode(descriptor, options, pluginContext));
  }
...
function genTemplateCode(descriptor, options, pluginContext) {
  ...
  if (!template4.lang && !template4.src) {
    ...
    if (template4.src) {
      linkSrcToDescriptor(template4.src, descriptor);
    }
...
async function genScriptCode(descriptor, options) {
  ...
  if (script5) {
    if ((!script5.lang || script5.lang === "ts" && options.devServer) && !script5.src) {
      ...
      if (script5.src) {
        linkSrcToDescriptor(script5.src, descriptor);
      }
...
function genStyleCode(descriptor) {
  let stylesCode = ``;
  let hasCSSModules = false;
  if (descriptor.styles.length) {
    descriptor.styles.forEach((style2, i) => {
      if (style2.src) {
        linkSrcToDescriptor(style2.src, descriptor);
      }
...
function genCustomBlockCode(descriptor) {
  let code = "";
...
function linkSrcToDescriptor(src, descriptor) {
  const srcFile = path5.default.posix.resolve(path5.default.posix.dirname(descriptor.filename), src);
